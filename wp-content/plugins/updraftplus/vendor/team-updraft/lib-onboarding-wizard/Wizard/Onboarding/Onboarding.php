<?php
namespace Updraftplus\Updraftplus\Wizard\Onboarding;

defined( 'ABSPATH' ) || die();

use Updraftplus\Updraftplus\Wizard\Installer\Installer;
use Updraftplus\Updraftplus\Wizard\RestResponse\RestResponse;
...
class Onboarding {
	...
	public function init(): void {
        ...
        if (empty($this->privacy_url_label)) {
            $this->privacy_url_label = __( 'Privacy Statement', 'updraftplus'  );
        }
			...
			if ( isset( $step['fields'] ) && is_array( $step['fields'] ) ) {
				foreach ( $step['fields'] as $field_index => $field ) {
					...
					if ( $field['type'] === 'email' ) {
						...
						if (empty($field['value'])) $field['value'] = $current_user_email;
					}
					if ( $field['id'] === 'plugins' ) {
                        ...
                        if ($this->is_all_plugins_installed){
                            ...
                            $steps[ $step_index ]['button']['label'] = __('Continue', 'updraftplus');
                        }
					}
	...
	/**
	 * Handle position update from request data.
	 * Can be called from update_settings (piggyback) or update_position (separate).
	 *
	 * @param array $data Request data containing step_id/step and open_group_id.
	 */
	private function handle_position_update( array $data ): void {
		// Support both 'step_id' (from update_position) and 'step' (from update_settings)
		$step_id       = isset( $data['step_id'] ) ? $this->sanitize_step_id( $data['step_id'] ) : '';
		if ( empty( $step_id ) && isset( $data['step'] ) ) {
			$step_id = $this->sanitize_step_id( $data['step'] );
		}
		$open_group_id = isset( $data['open_group_id'] ) ? sanitize_text_field( $data['open_group_id'] ) : '';
		
		if ( $step_id !== '' ) {
			$this->save_onboarding_position( $step_id, $open_group_id );
		}
	}
	...
	private function handle_onboarding_action( string $action, array $data ): \WP_REST_Response {
		...
		switch ( $action ) {
            case 'user_skipped_wizard':
				...
				do_action( $this->prefix . '_skipped_onboarding' );
				$this->clear_onboarding_position();
                $message = __('User skipped the wizard', 'updraftplus');
                $response = $this->response( true, [], $message);
                break;
            case 'user_completed_wizard':
                update_site_option($this->prefix . '_completed_onboarding', true );
				do_action( $this->prefix . '_completed_onboarding' );
				$this->clear_onboarding_position();
                $message = __('User Completed the wizard', 'updraftplus');
                $response = $this->response( true, [], $message);
                break;
			...
			case 'update_settings':
				...
				// Piggyback position save: persist step + accordion state alongside settings
				$this->handle_position_update( $data );
	...
	/**
	 * Save the onboarding position (current step and open accordion group)
	 * as a site option so it can be restored after a page redirect (e.g. OAuth flow).
	 *
	 * @param string $step_id       The ID of the current step.
	 * @param string $open_group_id The ID of the currently open accordion group (empty string if none).
	 */
	private function save_onboarding_position( string $step_id, string $open_group_id ): void {
		update_site_option(
			"{$this->prefix}_onboarding_position",
			array(
				'step_id'       => $step_id,
	...
	/**
	 * Get the saved onboarding position.
	 *
	 * @return array{step_id: string, open_group_id: string}|null
	 */
	private function get_onboarding_position(): ?array {
		$position = get_site_option( "{$this->prefix}_onboarding_position" );
		if ( ! is_array( $position ) || empty( $position['step_id'] ) ) {
			return null;
		}
		return $position;
	}
	...
	/**
	 * Clear the saved onboarding position (called when the wizard is completed or skipped).
	 */
	private function clear_onboarding_position(): void {
		delete_site_option( "{$this->prefix}_onboarding_position" );
	}
	...
	public function enqueue_onboarding_scripts(): void {
		...
		wp_set_script_translations( 'teamupdraft_onboarding', 'updraftplus', $this->languages_dir );
        ...
        wp_localize_script(
			...
			[
                ...
                'exit_wizard_text'      => !empty($this->exit_wizard_text) ? $this->exit_wizard_text : __('Exit setup', 'updraftplus'),
                'onboarding_position'   => $this->get_onboarding_position(),
